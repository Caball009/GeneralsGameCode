...
void AudioEventRTS::generateFilename( void )
{
	...
	if (m_eventInfo->m_soundType == AT_Music || m_eventInfo->m_soundType == AT_Streaming) {
		...
		if (BitIsSet(m_eventInfo->m_control, AC_RANDOM))
		{
			if (m_isLogicalAudio)
			{
				which = GameLogicRandomValueUnsynchronized(0, m_eventInfo->m_sounds.size() - 1);
			}
			else
			{
...
void AudioEventRTS::generatePlayInfo( void )
{
	...
	if (attackSize > 0) {
		...
		Int attackToPlay;
		if (m_isLogicalAudio) {
			attackToPlay = GameLogicRandomValueUnsynchronized(0, attackSize - 1);
		} else {
	...
	if (decaySize > 0) {
		...
		Int decayToPlay;
		if (m_isLogicalAudio) {
			decayToPlay = GameLogicRandomValueUnsynchronized(0, decaySize - 1);
		} else {
