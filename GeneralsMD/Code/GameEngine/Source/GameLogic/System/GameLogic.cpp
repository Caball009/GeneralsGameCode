...
void GameLogic::writeCRCBuffersToDisk(UnsignedInt frame) const
{
	...
	if (fp)
	{
		constexpr const char version[] = "[ DEEP CRC DATA (VERSION 0.0.2) ]";
		...
		size_t start = frame - frameCount;
		size_t end = frame + frameCount;

		if (TheGameLogic->getFrame() < ARRAY_SIZE(m_crcBuffers))
		{
			start = 0;
			end = ARRAY_SIZE(m_crcBuffers);
		}

		for (size_t i = start, j = 0; i < end && j < ARRAY_SIZE(m_crcBuffers); ++i, ++j)
		{
			const UnsignedInt index = i % ARRAY_SIZE(m_crcBuffers);
