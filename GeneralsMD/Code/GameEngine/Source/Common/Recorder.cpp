...
{
	static_assert(ARRAY_SIZE(m_playerData) == MAX_PLAYER_COUNT
		&& ARRAY_SIZE(m_inactivePlayer) == MAX_PLAYER_COUNT, "array sizes must be equal to player count");

	memset(m_inactivePlayer, 0, sizeof(m_inactivePlayer));
}
...
void RecorderClass::CRCInfo::init(Bool isMultiplayer, Int localPlayerIndex)
{
	...
	for (size_t i = 0; i < MAX_PLAYER_COUNT; ++i)
	{
		...
		m_inactivePlayer[i] = false;
	}
}

void RecorderClass::CRCInfo::addPlaybackCRC(UnsignedInt val)
{
...
void RecorderClass::CRCInfo::addPlayerCRC(Int playerIndex, UnsignedInt val)
{
...
RecorderClass::CRCInfo::MismatchData RecorderClass::CRCInfo::getMismatchData()
{
	CRCInfo::MismatchData mdata;

	for (UnsignedInt largestQueueSize = getLargestQueueSize(), j = 0; j < largestQueueSize; ++j)
	{
		const UnsignedInt playbackCRC = getPlaybackCRC();
		...
		for (size_t i = 0; i < MAX_PLAYER_COUNT; ++i)
		{
			...
			if (!m_inactivePlayer[i] && playbackCRC == playerCRC)
				continue;
			...
			if (!mdata.mismatched)
			{
				...
				{
					mdata = CRCInfo::MismatchData(static_cast<Byte>(i), static_cast<UnsignedShort>(m_playbackData.size()), playbackCRC, playerCRC);
				}
			}
		}
...
UnsignedInt RecorderClass::CRCInfo::getPlaybackCRC()
{
...
{
	const CRCInfo::MismatchData mdata = m_crcInfo.getMismatchData();
	if (mdata.mismatched)
	{
		...
		const UnsignedInt mismatchFrame = TheGameLogic->getFrame() - mdata.queueSize - 1;

		// Now also prints a UI message for it.
		const Player* player = ThePlayerList->getNthPlayer(mdata.playerIndex);
		...
		TheInGameUI->message(mismatchDetailsStr, mdata.playbackCRC, mdata.playerCRC, mismatchFrame,
			player ? player->getPlayerDisplayName().str() : L"Unknown");

		DEBUG_LOG(("Replay has gone out of sync!\nInGame:%8.8X Replay:%8.8X\nFrame:%d\nPlayer:%ls",
			mdata.playbackCRC, mdata.playerCRC, mismatchFrame, player ? player->getPlayerDisplayName().str() : L"Unknown"));
		...
		printf("CRC Mismatch in Frame %d, Local PlayerIndex %d, Mismatch PlayerIndex %d\n",
			mismatchFrame, m_crcInfo.getLocalPlayerIndex(), mdata.playerIndex);
